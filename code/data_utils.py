...
def import_data_dsn(root, num_bins):
    """
    Function: Import whole data (train, val and test) using pickle file (shuffled) for HistoNet DSN
    ...
    Output:
    train_set: train dataset list containing images, redundant count maps, count, size histogram_dsn1, size histogram_dsn2, size histogram 
    val_set: val dataset list containing images, redundant count maps, count, size histogram_dsn1, size histogram_dsn2, size histogram 
    test_set: test dataset list containing images, redundant count maps, count, size histogram_dsn1, size histogram_dsn2, size histogram 
    data_mean: Mean image of training dataset 
    """
    if os.path.exists(root + '/val.p'):
        print('Loading Val Datasets....')
    ...
    if num_bins[0] == 2 and num_bins[1] == 4 and num_bins[2] == 8:
        k1, k2, k3 = 3, 4, 5
    elif num_bins[0] == 4 and num_bins[1] == 8 and num_bins[2] == 16:
        k1, k2, k3 = 4, 5, 6
    else:
        print("Error: wrong value for num of bins !!")
    ...
    np_train_dataset_y = np.asarray([d[1] for d in train_data], dtype=theano.config.floatX)
    np_train_dataset_c = np.asarray([d[2] for d in train_data], dtype=theano.config.floatX)
    np_train_dataset_s_dsn1 = np.asarray([d[k1] for d in train_data], dtype=theano.config.floatX)
    np_train_dataset_s_dsn2 = np.asarray([d[k2] for d in train_data], dtype=theano.config.floatX)
    np_train_dataset_s = np.asarray([d[k3] for d in train_data], dtype=theano.config.floatX)

    np_val_dataset_x = np.asarray([d[0] for d in val_data], dtype=theano.config.floatX)
    np_val_dataset_y = np.asarray([d[1] for d in val_data], dtype=theano.config.floatX)
    np_val_dataset_c = np.asarray([d[2] for d in val_data], dtype=theano.config.floatX)
    np_val_dataset_s_dsn1 = np.asarray([d[k1] for d in val_data], dtype=theano.config.floatX)
    np_val_dataset_s_dsn2 = np.asarray([d[k2] for d in val_data], dtype=theano.config.floatX)
    np_val_dataset_s = np.asarray([d[k3] for d in val_data], dtype=theano.config.floatX)

    np_test_dataset_x = np.asarray([d[0] for d in test_data], dtype=theano.config.floatX)
    np_test_dataset_y = np.asarray([d[1] for d in test_data], dtype=theano.config.floatX)
    np_test_dataset_c = np.asarray([d[2] for d in test_data], dtype=theano.config.floatX)
    np_test_dataset_s_dsn1 = np.asarray([d[k1] for d in test_data], dtype=theano.config.floatX)
    np_test_dataset_s_dsn2 = np.asarray([d[k2] for d in test_data], dtype=theano.config.floatX)
    np_test_dataset_s = np.asarray([d[k3] for d in test_data], dtype=theano.config.floatX)
    ...
    train_set = [np_train_dataset_x, np_train_dataset_y, np_train_dataset_c,  np_train_dataset_s_dsn1, np_train_dataset_s_dsn2, np_train_dataset_s]
    val_set = [np_val_dataset_x, np_val_dataset_y, np_val_dataset_c,  np_val_dataset_s_dsn1, np_val_dataset_s_dsn2, np_val_dataset_s]
    test_set = [np_test_dataset_x, np_test_dataset_y, np_test_dataset_c,  np_test_dataset_s_dsn1, np_test_dataset_s_dsn2, np_test_dataset_s]
    return train_set, val_set, test_set, data_mean
